...
const LoginPage = () => {
    ...
    const onSubmit = async (data: LoginFormData) => {
        try {
            ...
            const user = result.data?.user || result.data?.userInfo;
            const accessToken = result.data?.accessToken || result.data?.createToken;

            if (result.success && user && accessToken) {
                dispatch(
                    setCredentials({
                        user,
                        accessToken,
                        ...
                        <div className="relative">
                            <div className="relative">
                                <input
                                    ...
                                    id="password"
                                    className="w-full px-4 py-3 border border-[#5E5E5E] rounded-[18px] bg-white focus:outline-none focus:ring-2 focus:ring-[#3CB371] focus:border-transparent pr-10 peer"
                                    placeholder=" "
                                />
                                <label
                                    htmlFor="password"
